// // CRON 2
// // This Cron job is run less Frequently than Cron1 
// // It updates the stock data in the MongoDB for the other endpoints
...
// //Continuing on from the 2222nd stock
// async function processStocksInBatch(stocks, startIndex, apiKey, apiHost, apiHost2) {
//   const totalStocks = stocks.length;
//   let processedStocks = 0; // Initialize processed stocks counter

//   // Start loop from startIndex
//   for (let i = startIndex; i < stocks.length; i++) {
//       const stock = stocks[i];
//       await processStock(stock.Stock, ['get-profile', 'get-income', 'get-news', 'get-cashflow', 'get-balance', 'historic', 'keyStatistics'], apiKey, apiHost, apiHost2);
//       processedStocks++; // Increment the counter after each stock is processed
//       const percentageProcessed = ((processedStocks / (totalStocks - startIndex)) * 100).toFixed(2); // Calculate the percentage of stocks processed
//       console.log(`${percentageProcessed}% of stocks processed (${processedStocks}/${totalStocks - startIndex}) starting from index ${startIndex}`);
//       await delay(250); // Ensure to respect the 4 requests per second rate limit
//   }
// }
...
//         //Continue from the 2327 stock
//         const stocks = await Stock.find({}).exec(); // Fetch all stocks
//         const startIndex = 3517; // Start from the 1350th stock (0-indexed)

//         // Process stocks respecting the API rate limit
//         // await processStocksInBatch(stocks, apiKey, apiHost, apiHost2);
//         await processStocksInBatch(stocks, startIndex, apiKey, apiHost, apiHost2);
