            ...
            {({ open }) => (
              <Fragment>
               <div className="relative w-40">
                ...
                {open && (
                  <Listbox.Options className="listbox-options absolute mt-2 w-full z-10 overflow-auto scrollbar scrollbar-track-gray-100 scrollbar-thumb-gray-900" style={{ maxHeight: '13rem' }}>
    ...
    {({ open }) => (
      <>
        <div className="relative w-full">
          <Listbox.Button className="dropdown-button flex justify-between items-center w-full h-10 px-3 shadow-md rounded-md">
             <span>Filters</span> 
            {open ? <FaChevronUp className="h-4 w-4 text-gray-500" /> : <FaChevronDown className="h-4 w-4 text-gray-500" />}
          </Listbox.Button>
          {open && (
            <Listbox.Options className="listbox-options absolute mt-2 w-full z-10 bg-white overflow-auto shadow-lg rounded-md p-4" style={{maxHeight: '20rem'}}>
              <div className="text-gray-700 text-sm mb-4">Select Industry:</div>
              <div className="grid grid-cols-2 gap-4">
                ...
                <Listbox.Option key={industry.id} value={industry} as={Fragment}>
                  {({ active, selected }) => (
                    <div 
                      className={`flex justify-start items-center text-xs cursor-pointer rounded-full border px-4 py-2 ${selected ? 'bg-blue-500 text-white' : 'bg-gray-100 text-black'}`}
                      style={{ 
                        backgroundColor: selected ? 'rgba(94, 93, 240, 0.5)' : '', 
                        borderColor: active ? 'rgba(94, 93, 240, 0.5)': 'transparent', // Highlight border when active
              ...
              {industries.length > 6 && (
                <div className="flex flex-col items-center mt-2">
                  <button className="text-center m-4 text-grey-500 text-xs" onClick={toggleViewAll}>
